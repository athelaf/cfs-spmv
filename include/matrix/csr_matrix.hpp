...
#define HYB_BW_THRESHOLD 10000
#define BALANCING_STEPS 10
#define IMBALANCE_TOL 0
#define MAX_COLORS 128 // Padded to 64 bytes to avoid false sharing
#ifdef _USE_DOUBLE
#define BLK_FACTOR 8
#define BLK_BITS 3
#else
#define BLK_FACTOR 16
#define BLK_BITS 4
#endif
...
public:
  SymmetryCompressionData()
      : rowptr_(nullptr), rowind_(nullptr), colind_(nullptr), values_(nullptr),
        diagonal_(nullptr), range_ptr_(nullptr), range_start_(nullptr),
        range_end_(nullptr), rowptr_h_(nullptr), colind_h_(nullptr),
        values_h_(nullptr), local_vector_(nullptr), platform_(Platform::cpu) {}

  SymmetryCompressionData(Platform platform)
      : rowptr_(nullptr), rowind_(nullptr), colind_(nullptr), values_(nullptr),
        diagonal_(nullptr), range_ptr_(nullptr), range_start_(nullptr),
        range_end_(nullptr), rowptr_h_(nullptr), colind_h_(nullptr),
        values_h_(nullptr), local_vector_(nullptr), platform_(platform) {}
...
class CSRMatrix : public SparseMatrix<IndexT, ValueT> {
  typedef tbb::concurrent_vector<tbb::concurrent_unordered_set<IndexT>>
      ConflictGraph;
...
public:
  ...
  CSRMatrix(const string &filename, Platform platform = Platform::cpu,
    ...
    nnz_low_ = nnz_diag_ = nrows_left_ = 0;
    ...
    if (symmetric_) {
      ...
      if (nthreads_ == 1) {
        ...
        if (atomics_)
          ...
          spmv_fn =
              bind(&CSRMatrix<IndexT, ValueT>::cpu_mv_sym_conflict_free_v1,
                   this, _1, _2);
        else if (conflict_free_aposteriori_ && !hybrid_ && part_by_nnz_)
          spmv_fn =
              bind(&CSRMatrix<IndexT, ValueT>::cpu_mv_sym_conflict_free_v2,
                   this, _1, _2);
        else
          assert(false);
      }
...
private:
  ...
  int nnz_low_, nnz_diag_, nrows_left_;
  ...
  if (part_by_nnz_ || part_by_ncnfls_) {
    ...
    for (int i = 0; i < nrows_; ++i) {
      for (int j = rowptr_[i]; j < rowptr_[i + 1]; ++j) {
        if (abs(colind_[j] - i) < HYB_BW_THRESHOLD ||
...
void CSRMatrix<IndexT, ValueT>::partition_by_conflicts(int nthreads) {
  ...
  int V = (int)ceil(nrows_ / (double)BLK_FACTOR);
  ...
  if (BLK_FACTOR == 1) {
    ...
    xadj[0] = 0;
    for (int i = 0; i < nrows_; i += BLK_FACTOR) {
      for (int k = i; k < min(i + BLK_FACTOR, nrows_); ++k) {
        for (int j = rowptr_[k]; j < rowptr_[k + 1]; ++j) {
          tmp.insert(colind_[j] >> BLK_BITS);
        }
      }

      for (auto &elem : tmp)
        v_adjncy.push_back(elem);
      xadj[(i >> BLK_BITS) + 1] = xadj[(i >> BLK_BITS)] + tmp.size();
      vwgt[i >> BLK_BITS] = rowptr_[min(i + BLK_FACTOR, nrows_)] - rowptr_[i];
      tmp.clear();
    }
  ...
  row_subset_.resize(nthreads_);
  for (int i = 0; i < nrows_; ++i) {
    row_subset_[row_part_[i >> BLK_BITS]].push_back(i);
  }
...
void CSRMatrix<IndexT, ValueT>::conflict_free_apriori() {
  ...
  nnz_low_ = values_sym.size();
  colind_sym_ = (IndexT *)internal_alloc(nnz_low_ * sizeof(IndexT), platform_);
  values_sym_ = (ValueT *)internal_alloc(nnz_low_ * sizeof(ValueT), platform_);
...
void CSRMatrix<IndexT, ValueT>::conflict_free_aposteriori() {
        ...
        for (int j = first_upper + 1; j < rowptr_[i + 1]; ++j) {
          ...
          for (int k = first_upper; k < j; ++k) {
            ...
            if (row_part_[prev_row] != row_part_[cur_row]) {
              int prev_blk_row = prev_row >> BLK_BITS;
              int cur_blk_row = cur_row >> BLK_BITS;
  ...
  bool cnfls[MAX_COLORS][MAX_THREADS][MAX_THREADS];
  ...
  #pragma omp parallel num_threads(nthreads_)
  {
    ...
    if (part_by_ncnfls_) {
      int nrows = data->nrows_;
      for (int i = 0; i < nrows; ++i) {
        rowind[color_map[data->rowind_[i] >> BLK_BITS]].push_back(i);
      }
    } else {
      for (int i = row_split_[tid]; i < row_split_[tid + 1]; i++) {
        rowind[color_map[i >> BLK_BITS]].push_back(i);
...
void CSRMatrix<IndexT, ValueT>::color_dsatur(const ConflictGraph &g,
  ...
  vector<map<int, unordered_set<int>, greater<int>>> Q(MAX_THREADS);
  for (int i = 0; i < V; i++) {
  ...
  if (balance) {
    ...
    #pragma omp parallel num_threads(nthreads_)
    {
      ...
      if (part_by_ncnfls_) {
        int nrows = (int)ceil(data->nrows_ / (double)BLK_FACTOR);
        for (int i = 0; i < nrows; ++i) {
          int row = data->rowind_[i << BLK_BITS] >> BLK_BITS;
        ...
        int row_offset = row_split_[tid];
        for (int i = 0; i < nrows; ++i) {
          int row = ((i << BLK_BITS) + row_offset) >> BLK_BITS;
          if (color[row] < k_color) {
            total_load += v[row].weight;
          }
          load[color[row]] += v[row].weight;
      ...
      for (int step = 0; step < ncolors_; ++step) {
        ...
        while (load[max_c] - mean_load > IMBALANCE_TOL && !bin[max_c].empty()) {
...
// }
    ...
    int i = 0;
    while (abs(load[max_c] - mean_load) > IMBALANCE_TOL &&
           !bin[max_c].empty()) {
